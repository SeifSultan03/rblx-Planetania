...
script.Parent.MouseButton1Down:Connect(function()
	...
	debounce = true
	if not player.leaderstats:GetAttribute("BoughtEgg1Before") then
		-- free 10x luck
		if player.leaderstats.Coins.Value >= 1000 then
			script.Parent.Parent["2"].Visible = true
			wait(5)
			script.Parent.Parent["2"]:Destroy()
			
			local pet = Egg1:InvokeServer()
			script.Parent.Parent.Visible = false
			Camera.CameraType = Enum.CameraType.Scriptable
			Camera.CFrame = EggStuff:WaitForChild("Cam").CFrame
			local egg = EggStuff.Egg1.Egg -- CHANGE TO WHATEVER EGG NUMBER
