...
script.Parent.Touched:Connect(function(hit)
	...
	if hit.Parent:FindFirstChild("Humanoid") then
		...
		if player.CurrentPet:FindFirstChildOfClass("Model") then
			Multi = tonumber(player.CurrentPet:FindFirstChildOfClass("Model"):GetAttribute("Multi"))
		end
		
		script.Parent.Transparency = 1
		debounce = true
		player.leaderstats:SetAttribute("XP", player.leaderstats:GetAttribute("XP") + Multi)
		
		if remainder == 0 and current ~= 0 then
			-- give player 25 coins and fireclient with the num
			player.leaderstats.Coins.Value += 25 * Multi
			coinEvent:FireClient(player, 2.25 ,Multi * 25)
		else
			-- give player 1 coin fireclient with GUI for 1 coin
			coinEvent:FireClient(player, 1 + remainder/4 ,Multi)
			player.leaderstats.Coins.Value += Multi
